...
interface AsiaScreenProps {
  onBack: () => void;
}

const AsiaScreen: React.FC<AsiaScreenProps> = ({ onBack }) => {
  const [selectedPais, setSelectedPais] = useState<string | null>(null);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  const paises = [
    { nome: 'Líbano', cor: 'rgb(24, 99, 173)' },
    { nome: 'Japão', cor: 'rgb(24, 99, 173)' },
    { nome: 'Jerusalém', cor: 'rgb(24, 99, 173)' }
  ];

  const fotoPorPais: { [key: string]: { numeroFoto: number; nomeJornal: string }[] } = {
    'Líbano': [
      { numeroFoto: 1, nomeJornal: 'Le Jour - Líbano' }
    ],
    'Japão': [
      { numeroFoto: 2, nomeJornal: 'The Japan Time - Japão' }
    ],
    'Jerusalém': [
      { numeroFoto: 3, nomeJornal: 'The Jerusalem - Jerusalém' }
    ]
  };
      ...
      {selectedPais && fotosAtuais.length > 0 && (
        <div style={{
          ...
          <div style={{
            ...
            <img
              src={`/images/Asia/foto${fotoAtual.numeroFoto}.jpg`}
